...
def _get_surname_metaphone(db, surname: str) -> str | None:
    ...
    result = db.query_one(
        "SELECT SurnameMP FROM NameTable WHERE Surname = ? COLLATE RMNOCASE LIMIT 1",
        (surname,)
    )
        ...
        with ctx.get_database() as db:
            ...
            if name:
                ...
                if not results and not exact and surname:
                    ...
                    if surname_mp:
                        console.print(f"[dim]No exact matches. Trying phonetic search...[/dim]\n")
                        phonetic_results = queries.search_primary_names_phonetic(
                            surname_phonetic=surname_mp,
                            limit=limit
                        )
